...
// Based on generalized D3 chart - http://bost.ocks.org/mike/chart/

// TODO: Better range slider - http://square.github.com/crossfilter/
...
// Reusable Faceted Timeline Chart defintion
// Usage:
function facetedTimeline() {
	// default values
	var width = 600,
		height = 250,
...
function createVis(height, requestedTextSize, shouldDisplayExtras) {
	...
	if (displayExtras) {
		...
		$("#svg").parent().append("<button style='position:absolute; left:10px; top:" + (height-40) + "px;' onclick='window.open(\"http://hci-user-advocate.blogspot.com/2012/09/d3-visualization-of-all-my-publications.html\", \"_blank\")'>About</button>");
	} else {
		$("#svg").parent().prepend("<button style='position:absolute' onclick='window.location=\"papers/index.html\"'>Full Vis</button>");
	}
...
function yearMousemoveHandler() {
	...
	if (barActiveYearButton == 0) {
		...
		var newyear = Math.floor(barxscale.invert(newx));
		new_min_year = newyear;
	} else {
		// Dragging max year button
		if (newx < (barMinYearX + 16)) {
			newx = barMinYearX + 16;
		}
		var newyear = Math.ceil(barxscale.invert(newx));
		new_max_year = newyear;
	}
